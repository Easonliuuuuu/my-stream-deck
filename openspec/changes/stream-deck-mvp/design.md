...
- **`AudioDeviceCmdlets` corrupts non-Latin device names** → confirmed on real hardware: its `.Name` property silently mangles jack names outside the system's ANSI codepage (e.g. Chinese "喇叭"/"麥克風"), before the string ever reaches our script — no output-encoding fix can recover data already lost upstream. Mitigation applied: `Get-AudioDevices.ps1` reads the jack name from the registry (`PKEY_Device_FriendlyName`, keyed by the device ID the module *does* return intact) and reattaches the product-name suffix parsed from `.Name`.
- **A third-party gamepad tool (e.g. DS4Windows) can hold the DualSense's HID interface** → if such a tool is running, `node-hid` may not see the controller at all even though Windows reports it connected. No clean programmatic fix identified yet (quitting the tool didn't reliably release it in testing); likely needs either an admin-elevated device reset or the tool's own "release device" option. Flagged as an open risk for whoever completes task 5.3.
